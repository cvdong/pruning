...
# Load a model
yolo = YOLO("/home/DONG/PRUNE/ultralytics/runs/detect/train2/weights/last.pt")  # build a new model from scratch
model = yolo.model
...
def prune_conv(conv1: Conv, conv2: Conv):
    ...
    n = len(keep_idxs)
    print(f"剪枝比例： {(1 - n / len(gamma)) * 100} %")
...
yolo.train(data="XX.yaml", epochs=100, batch=32)
